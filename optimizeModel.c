  ...
  */  
  ...
  do
    {           
	  ...
	  l=0;
		while(!feof(ratesfile))
			{
			fscanf(ratesfile, "%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", &tr->partitionData[l].substRates[0],&tr->partitionData[l].substRates[1],&tr->partitionData[l].substRates[2],&tr->partitionData[l].substRates[3],&tr->partitionData[l].substRates[4],&tr->partitionData[l].substRates[5]);
			l++;
			}
      fclose(ratesfile);

      /*
        tr->partitionData[0].substRates[0] = 3.033113;
	  tr->partitionData[0].substRates[1] = 6.058146;
	  tr->partitionData[0].substRates[2] = 3.619107;
	  tr->partitionData[0].substRates[3] = 0.913196;
	  tr->partitionData[0].substRates[4] = 14.642717;
	  tr->partitionData[0].substRates[5] = 1;
      */
	...
	case GAMMA:      
	  /*optAlpha(tr, modelEpsilon, alphaList);*/ 
	  alphafile=fopen("alpha.txt", "r");
	    l=0;
		while(!feof(alphafile))
			{
			fscanf(alphafile, "%lf\n", &tr->partitionData[l].alpha);
			l++;
			}
	  fclose(alphafile);
