				   ...
				   const unsigned int *bitMask)
{
  ...
  freqfile=fopen("frequencies.txt", "r");
	l=0;
	while(!feof(freqfile))
		{
		if(l==model)
			{
			fscanf(freqfile, "%lf\t%lf\t%lf\t%lf\n", &pfreqs[0],&pfreqs[1],&pfreqs[2],&pfreqs[3]);
			/*printf("frequencies: %lf\t%lf\t%lf\t%lf\n", pfreqs[0],pfreqs[1],pfreqs[2],pfreqs[3]);*/
			}
		else
			{
			/*printf("here\n");*/
			while(!feof(freqfile) && getc(freqfile) != '\n');
			}
		l++;
		}
  fclose(freqfile);
